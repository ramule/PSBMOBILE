...
//	public final boolean deliveryRequired = false;
        ...
        cordova.getThreadPool().execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    Integer subscriptionId = Integer.parseInt(args.getString(2));
    ...
    private void send(final CallbackContext callbackContext, String phoneNumber, String message, Integer subscriptionId) {
        ...
        final BroadcastReceiver broadcastReceiverDeliver = new BroadcastReceiver() {
        ...
        String intentFilterSent, intentFilterDeliver;
        PendingIntent deliverIntent, sentIntent;

        intentFilterDeliver = INTENT_FILTER_SMS_DELIVERED + java.util.UUID.randomUUID().toString();
        this.cordova.getActivity().registerReceiver(broadcastReceiverDeliver, new IntentFilter(intentFilterDeliver));
        deliverIntent = PendingIntent.getBroadcast(this.cordova.getActivity(), 0, new Intent(intentFilterDeliver), 0);

        intentFilterSent = INTENT_FILTER_SMS_SENT + java.util.UUID.randomUUID().toString();
        this.cordova.getActivity().registerReceiver(broadcastReceiver, new IntentFilter(intentFilterSent));
        sentIntent = PendingIntent.getBroadcast(this.cordova.getActivity(), 0, new Intent(intentFilterSent), 0);
        ...
        if (parts.size() > 1) {
            ...
            manager.getSmsManagerForSubscriptionId(subscriptionId).sendMultipartTextMessage(phoneNumber, null, parts, sentIntents, deliverIntents);
        } else {
            // Changed For Send sms from selected sim Author Vaibhav Khot
            // manager.sendTextMessage(phoneNumber, null, message, sentIntent, null);
            manager.getSmsManagerForSubscriptionId(subscriptionId).sendTextMessage(phoneNumber, null, message, sentIntent, deliverIntent);
        }
    }
}
