...
@Component({
  selector: 'app-open-deposit-account-success-receipt',
  templateUrl: './open-deposit-account-success-receipt.component.html',
  styleUrls: ['./open-deposit-account-success-receipt.component.scss']
})
export class OpenDepositAccountSuccessReceiptComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.receiptResp = this.DataService.openFDReceiptObj;
 ...
 */
    ...
    selectedFields += "Deposit Scheme :" + this.receiptResp.depositScheme == 'G' ? 'General' : 'Tax Saver' + ", ";
    ...
    selectedFields += "Maturity Amount :" + this.receiptResp.maturityAmount + ", ";
    ...
    selectedFields += "Maturity Instruction :" + this.receiptResp.maturityInstruction + ", ";
  ...
  downloadPdfReceipt(type) {
    ...
    this.receiptOpenFDJson[0].value = this.receiptResp.depositType ? this.receiptResp.depositType : '-';
    this.receiptOpenFDJson[1].value = this.receiptResp.depositScheme ? this.receiptResp.depositScheme == 'G' ? 'General' : 'Tax Saver' : '-';
    this.receiptOpenFDJson[2].value = this.receiptResp.depositorType ? this.receiptResp.depositorType : '-';
    this.receiptOpenFDJson[3].value = this.receiptResp.depositAmount ? this.receiptResp.depositAmount : '-';
    this.receiptOpenFDJson[4].value = this.receiptResp.tenure ? this.receiptResp.tenure : '-';
    this.receiptOpenFDJson[5].value = this.receiptResp.interestRate ? this.receiptResp.interestRate+' %' : '-';
    this.receiptOpenFDJson[6].value = this.receiptResp.maturityAmount ? '₹ '+ this.receiptResp.maturityAmount : '-';
    this.receiptOpenFDJson[7].value = this.receiptResp.maturityDate ? this.receiptResp.maturityDate : '-';
    this.receiptOpenFDJson[8].value = this.receiptResp.modeOfOperation ? this.receiptResp.modeOfOperation : '-';
    this.receiptOpenFDJson[9].value = this.receiptResp.maturityInstruction ? this.receiptResp.maturityInstruction : '-';
    this.receiptOpenFDJson[10].value = this.receiptResp.maturityPayoutAccount ? this.receiptResp.maturityPayoutAccount : '-';
    this.receiptOpenFDJson[11].value = this.receiptResp.nomineeName ? this.receiptResp.nomineeName : '-';
    ...
    this.generatePDF(imgColor, this.receiptType, this.DataService.receiptmsg, this.refTransJson, this.receiptOpenFDJson, 'Open_FD_Receipt', branchJSON, type, selAccDtl[0].accountNo, this.todayDateTime);
  }
  ...
  generatePDF(imageColor,msg,submsg,reftransJSON,receiptJSON,receiptName,branchJSON,printPDF, accountNo, todayDateTime) {
    ...
    if(this.DataService.FDRDAccNumber) {
      doc.setFontSize(9)
      doc.text( this.openFDReceiptObj.depositType  + ' Account Number: '+this.DataService.FDRDAccNumber, 40, 99, 'left');
