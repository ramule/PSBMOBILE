...
@Component({
  selector: 'app-pmjjby-record',
  templateUrl: './pmjjby-record.component.html',
  styleUrls: ['./pmjjby-record.component.scss']
})
export class PmjjbyRecordComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    var dateData = this.enrollmentDetailsData.joinDate.split('-')[1]
    if(+dateData>=5)
    {
      var newDate = +this.enrollmentDetailsData.joinDate.split('-')[2] + 1
      this.enrollmentDetailsData.tranDate = "31-05-"+newDate
    }
    else
    this.enrollmentDetailsData.tranDate = "31-05-"+this.enrollmentDetailsData.joinDate.split('-')[2]
    
   // this.enrollmentDetailsData.tranDate = 

    console.log('enrollment Details: ', this.enrollmentDetailsData);
 ...
 */
    ...
    selectedFields += "Scheme : Pradhan Mantri Jeevan Jyoti Bima Yojana(PMJJBY), ";
  ...
  downloadPdfReceipt(type?: any) {
    ...
    doc.text("Pradhan Mantri Jeevan Jyoti", 20, 85, 'left');
    ...
    if (type) {
      ...
      this.commonMethod.downloadPDF(doc, 'PMJJBY_Enrollment_Details');
    }
