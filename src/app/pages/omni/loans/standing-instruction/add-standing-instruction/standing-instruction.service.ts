...
export class StandingInstructionService {
  ...
  addStandingInstructionParam(enrollmentData, pensionData) {
    ...
    console.log(' addStandingInstructionParam ', JSON.stringify(inputData));
  ...
  getStandingInstructionService(formData,otpRequired){
      // var paymentFrequency = formData.paymentFrequency == 'W' ? 'Weekly' : formData.paymentFrequency == 'M' ? 'Monthly' :  formData.paymentFrequency == 'H' ? 'Half yearly' : formData.paymentFrequency == 'Q' ? 'Quartely' : 'Yearly'
      ...
      // this.amountModel.replaceAll('₹','').replace(/^\₹|,|\.\d*$/gm, '').trim()
