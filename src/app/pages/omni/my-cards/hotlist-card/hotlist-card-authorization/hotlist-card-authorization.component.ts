...
import { HotlistCardAuthorizationService } from '../hotlist-card-authorization/hotlist-card-authorization.service';
...
@Component({
  selector: 'app-hotlist-card-authorization',
  templateUrl: './hotlist-card-authorization.component.html',
  styleUrls: ['./hotlist-card-authorization.component.scss']
})
export class HotlistCardAuthorizationComponent implements OnInit {
  ...
  constructor(
    ...
    public hotlistCardAuthorizationService:HotlistCardAuthorizationService,
...
confirmOtpClick() {
  ...
  if (this.otpSessionForm.valid) {
   ...
   var param = this.hotlistCardAuthorizationService.getSendOTPSessionReq(otpValue);
...
submitReq()
{
  ...
  .subscribe((resp) => {
    if (resp.responseParameter.opstatus == '00') {
      console.log(resp);
      this.DataService.transactionReceiptObj.response = 'Successful';
    } else {
      this.DataService.transactionReceiptObj.response = 'Failed';
    }

    this.DataService.transactionReceiptObj.msg = resp.responseParameter.Result;
    this.DataService.transactionReceiptObj.rrn = resp.responseParameter.RRN =="" ? "-" : resp.responseParameter.RRN ;
    this.router.navigate(['/hotlistSuccess']);
...
resendOTP() {
  ...
  var resendOTPReq = this.hotlistCardAuthorizationService.getResendOTPSessionReq();
  this.http
    .callBankingAPIService(
      resendOTPReq,
      this.storage.getLocalStorage(this.constant.storage_deviceId),
